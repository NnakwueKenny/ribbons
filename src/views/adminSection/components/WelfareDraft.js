...
const WelfareDraft = () => {
    ...
    const getAlldrafts = () => {
        setIsLoading(true);
        fetch('http://localhost:3500/draft/get-drafts',
            {
                method: 'post',
                headers: {
        ...
        .then(data =>{
            console.log(data);
            setDrafts(data.filter(item => item.cat === 'welfare'));
            setIsLoading(false)
        })
    }
        ...
        <div className='relative w-full flex justify-center p-10'>
            {
                ...
                <div className='fixed top-0 left-0 w-full h-full bg-gray-600 bg-opacity-25 flex justify-center items-center p-6'>
                    <div className='relative bg-white w-full max-w-4xl h-full max-h-[500px] rounded-2xl p-6'>
                        <button onClick={() => setPreviewDraft(prevValue => !prevValue)} className='absolute top-8 right-8 text-2xl'><i className='fa fa-times'></i></button>
                        About Complaint
                    </div>
                </div>
            }
                        ...
                        {drafts.map((complaint, index) => {
                            return (
                                <div key={index+1} className="w-full h-80 flex flex-col p-4 bg-white border border-gray-200 rounded-lg shadow hover:shadow-md">
                                    ...
                                    <div className='px-5'>
                                        <button onClick={() => setPreviewDraft(prevValue => !prevValue)} id={complaint.id} className="flex w-full items-center justify-center px-3 py-3 text-lg font-semibold text-center text-white bg-purple-800 rounded-lg hover:bg-purple-900 focus:ring-4 focus:outline-none focus:ring-purple-300">
                                            View
