...
const AdminLogin = () => {
  ...
  const register = () => {
    ...
    .then(data => {
      console.log(data)
        if (data.err) {
          toggleMessageContent(setregisterMessage, data.err);
          setLoginStatus(false);
        } else {
          toggleMessageContent(setregisterMessage, data.message);
          setLoginStatus(true);
          setTimeout(() => {
      ...
      <div data-scroll className="promo relative w-full flex max-w-4xl flex-col items-center py-4 px-6 shadow rounded-3xl">
        ...
        <form id='login-form' className='font-orbitron w-full max-w-3xl p-6 md:py-8 md:px-10 md:p-6 bg-white bg-opacity-25 rounded-2xl'>
          ...
          <div className="relative z-0 mb-4 w-full group">
            ...
            <label htmlFor="location" className="flex peer-focus:font-medium absolute text-xl  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 text-gray-600 peer-focus:text-purple-900 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Location:</label>
          </div>
          { registerMessage !== '' &&
            <div className='login-message'>
              <span className={`login-message-content block text-start font-semibold pb-3 italic text-${loginStatus === 'success'? 'green-500': 'purple-800'}`}>{registerMessage}</span>
            </div>
          }
          <div className='py-4'>
